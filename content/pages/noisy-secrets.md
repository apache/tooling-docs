...
Draft Specification, 2026-05-06.
...
The Prefix production in Section 4.1 is only a syntactic fragment. It was chosen to be concise, and to make room for the Namespace String. The conceptual prefix of a Noisy Secret is the Prefix, Pad, Namespace length prefix, and Pad. This longer conceptual prefix is much more likely to distinguish Noisy Secrets from other strings. Scanners SHOULD therefore use the following specialised regexes for each length-prefixed subset as prefilters:
...
For the regexes above, the alphabets are manifested as:

    BASE37 = [0-9_a-z]
    BASE36 = [0-9a-z]
    BASE32 = [2-9a-km-np-z]

Because these regexes are only prefilters, they will also match strings that are not actually Noisy Secret Tags, such as ones where the final 8 `BASE37` characters are not the correct interleaved even and odd Reed-Solomon checksums for the preceding Namespace and Payload. They may also match syntactically valid NamespaceString values that could not have been produced from a real FQDN by the namespace construction algorithm. After a match has been validated as a Noisy Secret Tag, the implementation MAY obtain a Candidate Domain as described in Section 9.8 for reporting or routing purposes.
...
The values in this appendix are Noisy Secret Tags, not issued Noisy Secrets. They are test vectors for the construction and validation algorithms, with valid checksums, but their Payload values are fixed test inputs rather than values generated as specified in Section 5.5.
